...
void CSecureAppView::ConstructL(const TRect& aRect)
	...
	{
		SetRect(TRect(TPoint(0,0),TSize(800,200)));
	}
	else
	{
		SetRect(aRect);		
...
void CSecureAppView::Draw(const TRect& aRect) const
	...
	{   	
		...
		//TPoint pos2(10,50);
		//gc.DrawText(*(iEikonEnv->AllocReadResourceL(R_UNLOCK_TEXT)),pos2);	
	...
	TInt len=60;//length of text to be displayed in 1 line.
	TBool flag=0;
	...
	do
	{
		...
		gc.DrawText(text, TPoint(10,pos));
		pos = pos + 20;
	}
	while(flag == 0);
